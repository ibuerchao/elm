...
public class MyExampleGenerator extends ExampleGenerator {
    public MyExampleGenerator(String project) {
        super(project);
    }
    ...
    private InnerClass getGeneratedCriteriaInnerClass(
        ...
        method.addParameter(new Parameter(FullyQualifiedJavaType
                .getStringInstance(), "condition")); //$NON-NLS-1$
        method.addBodyLine("if (condition!=null && !condition.equals(\"\")){"); //$NON-NLS-1$
        ...
        method.addBodyLine("if (!(value instanceof String)){"); //$NON-NLS-1$
        method.addBodyLine("criteria.add(new Criterion(condition, value));"); //$NON-NLS-1$
        method.addBodyLine("}else if (!value.equals(\"\")){"); //$NON-NLS-1$
        method.addBodyLine("criteria.add(new Criterion(condition, value));"); //$NON-NLS-1$
        method.addBodyLine("}"); //$NON-NLS-1$
        method.addBodyLine("}"); //$NON-NLS-1$
        if (criteriaLists.size() > 1) {
